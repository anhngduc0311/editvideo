"""
translator.py - Multi-Engine SRT Subtitle Translation (DeepSeek API & Google Gemini API)
"""
...
def check_gemini_api_status(api_key: str, preferred_model: str = "gemini-3.5-flash-lite") -> dict:
    """
    Kiểm tra nhanh tình trạng hoạt động và Rate Limit của Gemini API Key.
    Thử nghiệm các model phổ biến để tìm ra model đang hoạt động tốt nhất.
    """
    if not api_key or not api_key.strip():
        return {
            "valid": False,
            "status": "INVALID_KEY",
            "message": "Chưa nhập API Key. Vui lòng nhập key trên giao diện.",
            "recommended_model": preferred_model,
            "working_models": [],
            "rate_limited_models": [],
            "model_results": {}
    ...
    def _test_single_model(model: str):
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                return model, {"code": 429, "latency_ms": latency, "msg": "Rate Limit (429 - Tạm hết quota)"}
            ...
            return model, {"code": -1, "latency_ms": 4000, "msg": "Timeout (4s)"}
        except Exception as e:
            return model, {"code": -1, "latency_ms": 0, "msg": f"Error: {e}"}

    with ThreadPoolExecutor(max_workers=min(6, len(unique_models))) as executor:
        futures_res = list(executor.map(_test_single_model, unique_models))
    ...
    has_auth_error = any(r.get("code") in (400, 403) for r in model_results.values()) and not working_models
    ...
    if working_models:
        ...
        for wm, _ in working_models:
            if wm == preferred_model:
                ...
                break
        rate_info = f" ({len(rate_limited_models)} model bị rate limit)" if rate_limited_models else ""
        msg = f"🟢 Gemini API Key HOẠT ĐỘNG TỐT! Có {len(working_models)} model sẵn sàng{rate_info}. Đề xuất: {rec_model}"
        return {
            "valid": True,
            "status": "OK",
            "message": msg,
        ...
        return {
            "valid": True,
            "status": "RATE_LIMITED",
            "message": "🟡 API Key hợp lệ nhưng các model chính đang bị RATE LIMIT (429)! Vui lòng chờ 1-2 phút hoặc đổi key.",
            "recommended_model": preferred_model,
            "working_models": [],
            "rate_limited_models": rate_limited_models,
            "model_results": model_results
        ...
        return {
            "valid": False,
            "status": "INVALID_KEY",
            "message": "🔴 API Key KHÔNG HỢP LỆ (Lỗi 400/403: API_KEY_INVALID). Vui lòng lấy key mới từ Google AI Studio.",
            "recommended_model": preferred_model,
            "working_models": [],
            "rate_limited_models": [],
            "model_results": model_results
        ...
        return {
            "valid": False,
            "status": "NETWORK_ERROR",
            "message": "⚠️ Không thể kết nối tới Google Gemini API (Kiểm tra internet/proxy/VPN).",
            "recommended_model": preferred_model,
            "working_models": [],
            "rate_limited_models": [],
            "model_results": model_results
...
class GeminiTranslator:
    """
    Module dịch thuật phụ đề từ Tiếng Trung sang Tiếng Việt bằng Gemini API.
    Bảo toàn 100% cấu trúc timeline (start --> end) và số thứ tự của file SRT.
    Hỗ trợ cả Google Generative AI SDK và Direct HTTPS REST Client (chống lỗi phụ thuộc môi trường).
    """
    ...
    def __init__(self, config: PipelineConfig):
        ...
        if not self.api_key:
            raise ValueError(
                "Chưa cấu hình GEMINI_API_KEY! Hãy truyền API key vào config hoặc gán biến môi trường GEMINI_API_KEY."
            )
        self.model_name = config.gemini_model_name
        self._sdk_model = None

        # Thử khởi tạo SDK nếu có
        try:
    ...
    def check_status(self) -> dict:
        """Kiểm tra API Key và tình trạng model cho instance này"""
        return check_gemini_api_status(self.api_key, self.model_name)

    def _clean_markdown_response(self, text: str) -> str:
        return clean_markdown_response(text)

    def parse_srt_string(self, srt_content: str) -> List[SubtitleItem]:
        return parse_srt_string(srt_content)

    def _call_gemini_api(self, prompt: str, timeout: int = 120, max_retries: int = 3) -> str:
        """Gọi Gemini API qua direct REST API với cơ chế thử lại (retry) và tự động chuyển model dự phòng (fallback)"""
        candidate_models = [
            self.model_name,
            "gemini-3.5-flash-lite",
            "gemini-flash-lite-latest",
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        logger.warning(f"Gemini model {model} trả về 404, chuyển model tiếp theo...")
                        last_error = f"Status 404: {res.text}"
                        break
                    elif res.status_code == 429:
                        logger.warning(f"Gemini model {model} bị rate limit (429), chờ thử lại (lần {attempt}/{max_retries})...")
                        time.sleep(2 * attempt)
                        if attempt >= 2:
                            logger.info(f"Model {model} chạm trần rate limit, tự động chuyển sang model dự phòng tiếp theo...")
                            last_error = f"Status 429: {res.text}"
                        ...
                        logger.warning(f"Gemini API {model} trả về status {res.status_code}, thử lại (lần {attempt}/{max_retries})...")
                        time.sleep(1.5 * attempt)
                        last_error = f"Status {res.status_code}: {res.text}"
                except (requests.exceptions.Timeout, requests.exceptions.ConnectionError) as e:
                    logger.warning(f"Kết nối tới Gemini ({model}) bị timeout/ngắt ({e}), đang thử lại (lần {attempt}/{max_retries})...")
                    last_error = str(e)
                    time.sleep(2 * attempt)
                except Exception as e:
                    last_error = str(e)
                    if "404" in str(e) or "not found" in str(e).lower() or "no longer available" in str(e).lower():
                        break
                    logger.warning(f"Gemini model {model} lỗi ({e}), thử lại...")
                    time.sleep(1.5 * attempt)

        raise RuntimeError(f"Lỗi Gemini Translation API: Tất cả model đều thất bại. Chi tiết: {last_error}")
    ...
    def translate_srt(
        ...
        """Dịch file SRT từ Tiếng Trung sang Tiếng Việt qua Gemini API (chia batch thông minh chống timeout)"""
        input_srt_path = Path(input_srt_path).resolve()
        output_srt_path = Path(output_srt_path).resolve()
        output_srt_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        total_items = len(original_items)
        logger.info(f"[Bước 3] Đang dịch {total_items} câu phụ đề bằng Gemini API ({self.model_name})...")
        ...
        with tqdm(total=total_items, desc="[Bước 3] Gemini Dịch thuật SRT", leave=False) as pbar:
            for b_idx, batch in enumerate(batches, 1):
                batch_srt_chunk = "\n\n".join([item.to_srt_block().strip() for item in batch])
                prompt = f"{system_instruction}\n\n[DỮ LIỆU PHỤ ĐỀ TIẾNG TRUNG]:\n{batch_srt_chunk}"

                try:
                    response_text = self._call_gemini_api(prompt, timeout=120)
                    translated_raw = clean_markdown_response(response_text)
                    batch_parsed = parse_srt_string(translated_raw)
                except Exception as e:
                    logger.error(f"Lỗi khi dịch batch {b_idx}/{total_batches}: {e}")
                ...
                if len(batch_parsed) == len(batch):
                    ...
                    logger.warning(
                        f"Batch {b_idx}/{total_batches}: Gemini trả về {len(batch_parsed)} câu (gốc có {len(batch)} câu). "
                        "Tự động căn chỉnh text theo timeline gốc..."
                    )
                    for i, orig_item in enumerate(batch):
                        v_text = batch_parsed[i].text.strip() if i < len(batch_parsed) else orig_item.text
...
def create_translator(config: PipelineConfig):
    """Factory function tạo Translator phù hợp dựa vào config (DeepSeek hoặc Gemini)"""
    ...
    if provider == "deepseek":
        ...
        # Fallback to DeepSeek if key exists, else Gemini
